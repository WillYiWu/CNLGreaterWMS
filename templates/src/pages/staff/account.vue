            ...
            <template v-if="props.row.id === editid">
              <q-td key="account_name" :props="props">
                <q-input
                  ...
                  :label="$t('staff.view_account.account_name')"
                  autofocus
            ...
            <template v-if="props.row.id === editid">
              <q-td key="client_id" :props="props">
                <q-input
                  ...
                  outlined
                  square
                  v-model="editFormData.client_id"
                  :options="account_type_list"
                  transition-show="scale"
                  transition-hide="scale"
                  :label="$t('staff.view_account.client_id')"
                  :rules="[val => (val && val.length > 0) || error2]"
                />
              </q-td>
            </template>
            <template v-else-if="props.row.id !== editid">
              <q-td key="client_id" :props="props">{{ props.row.client_id }}</q-td>
            </template>
            <template v-if="props.row.id === editid">
              <q-td key="client_secret" :props="props">
                <q-input
                  dense
                  outlined
                  square
                  v-model="editFormData.client_secret"
                  :options="account_type_list"
                  transition-show="scale"
                  transition-hide="scale"
                  :label="$t('staff.view_account.client_secret')"
                  :rules="[val => (val && val.length > 0) || error2]"
                />
              </q-td>
            </template>
            <template v-else-if="props.row.id !== editid">
              <q-td key="client_secret" :props="props">{{ props.row.client_secret }}</q-td>
            </template>
      ...
      <q-card class="shadow-24">
        ...
        <q-card-section style="max-height: 325px; width: 400px" class="scroll">
          <q-input
            ...
            outlined
            square
            v-model.trim="newFormData.account_name"
            :label="$t('staff.view_account.account_name')"
            autofocus
            :rules="[val => (val && val.length > 0) || error1]"
          ...
          <q-input
            ...
            outlined
            square
            v-model="newFormData.client_secret"
            :options="account_type_list"
            transition-show="scale"
            transition-hide="scale"
            :label="$t('staff.view_account.client_secret')"
...
export default {
  ...
  data () {
    return {
      ...
      account_type_list: [],
      columns: [
        { name: 'account_name', required: true, label: this.$t('staff.view_account.account_name'), align: 'left', field: 'account_name' },
        { name: 'client_id', label: this.$t('staff.view_account.client_id'), field: 'client_id', align: 'center' },
        { name: 'client_secret', label: this.$t('staff.view_account.client_secret'), field: 'client_secret', align: 'center' },

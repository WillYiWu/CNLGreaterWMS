...
function getBaseUrl (name) {
  ...
  try {
    ...
    xhr.send(null)
    if (xhr.status === okStatus && xhr.responseText.trim()) {
      const configuredUrl = new URL(xhr.responseText.trim())
      if (configuredUrl.protocol === 'http:' || configuredUrl.protocol === 'https:') {
        return configuredUrl.origin
      }
    }
  } catch (error) {
    // A local override is optional; same-origin is the safe default.
  }

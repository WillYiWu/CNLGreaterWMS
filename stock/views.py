...
class StockListViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.request.user:
            if id is None:
                return StockListModel.objects.filter(openid=self.request.auth.openid,onhand_stock__gt=0).order_by("onhand_stock")
            else:
                return StockListModel.objects.filter(openid=self.request.auth.openid, onhand_stock__gt=0,id=id).order_by("onhand_stock")
        else:
